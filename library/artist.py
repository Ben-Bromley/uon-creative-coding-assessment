...
class Artist:
  ...
  def create_artwork(self, file_path):
      y, sr, spectral_centroids, rms = self.audio_manager.analyze_audio(file_path)  # obtain the audio to be analysed
      artwork = self.audio_converter.convert_audio_to_visuals(spectral_centroids, rms) # perform the conversion
      return artwork
